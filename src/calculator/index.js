...
container.style.width = '320px'; // Adjust width as needed
  ...
  switch (button) {
    case '=':
      try {
        ...
        display.textContent = Number.isNaN(result) ? 'Error' : result;
      } catch (error) {
    ...
    default:
      ...
      if (/^[0-9.]+$/.test(button)) {
        if (display.textContent === '0' || display.textContent === 'Error') {
          display.textContent = button === '.' ? '0.' : button;
        } else {
